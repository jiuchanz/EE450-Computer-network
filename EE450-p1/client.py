...
def handle_io():
    ...
    try:
        ...
        if state_transition_table[current_state].has_key(msg_from_server) :
            ...
            network_output.appendleft(currrent_msg)
            return
        ...
        if(current_time-time1)<3 :
            return
        else:
            print("no response from server, attempt to resent current messge: %s" % current_msg )
            network_output.appendleft(currrent_msg)
        pass
    ...
    try:
        ...
        # Do some things
        if command_table.has_key(input) :
            current_msg=command_table
            network_output.appendleft(currrent_msg)
        return
    except IndexError:
